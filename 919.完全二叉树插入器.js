...
CBTInserter.prototype.insert = function (v) {
    function searchEachLevel(list, val) {
        let newList = [];
        ...
        for (let index = 0; index < list.length; index++) {
            ...
            if (element.left !== null) {
                newList.push(element.left);
            } else {
                element.left = new TreeNode(v);
                return element.val;
            }
            if (element.right !== null) {
                newList.push(element.right);
            } else {
                element.right = new TreeNode(v);
                return element.val;
            }
        }

        return searchEachLevel(newList, val)
    }
    return searchEachLevel([this.root], v);
